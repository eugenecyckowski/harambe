...
PRIMING_PROMPT_BASE='Read .harambe-brief.md and execute it. If .harambe-worktree-notes.md exists in this worktree, read it first — it contains repo-level dev-env context (ports, gotchas, how to bring up your stack). Do NOT assume containers running from docker ps are yours; each worktree has its own docker-compose project, and the main repo or sibling worktrees may have their own stacks on different ports. Use `bin/wealthbox status` from INSIDE this worktree to see what is actually yours. Do NOT create cron jobs, scheduled remote triggers (schedule skill), or recurring loops — those are the orchestrator Harambe '"'"'s call to set up, not yours. When done, write WORKER_REPORT.md in this worktree root with: status (done/blocked/failed), one-line summary, changed files, any notes the user should see.'
